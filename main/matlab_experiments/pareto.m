...
for w = 1:num_weights
    ...
    for trial = 1:mc
        ...
        for a = 1:numel(algorithms)
            ...
            [gamma, ~, sum_rate_mc(trial, a)] = sinr_rate(cfg, h_eff, p_dbw);

            xi = semantic_map(gamma, cfg.m_k, semantic_mode, table_path, ...
                struct('a', cfg.proxy_a, 'b', cfg.proxy_b));

            [avg_qoe_mc(trial, a), ~, meta] = qoe(cfg, gamma, cfg.m_k, xi, weights);

            deadline_vio_mc(trial, a) = 1 - meta.deadline_ok;
            semantic_vio_mc(trial, a) = 1 - meta.semantic_ok;
        end
    end
