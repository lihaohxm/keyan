...
for w = 1:num_weights
    ...
    for trial = 1:mc
        ...
        for a = 1:numel(algorithms)
            alg = algorithms{a};
            assign = pick_assignment(cfg, ch, alg, cfg.p_dbw_list(1), semantic_mode, table_path, weights, trial_seed);
            h_eff = effective_channel(cfg, ch, assign);
            [gamma, ~, sum_rate_mc(trial, a)] = sinr_rate(cfg, h_eff, cfg.p_dbw_list(1));
            xi = semantic_map(gamma, cfg.m_k, semantic_mode, table_path, struct('a', cfg.proxy_a, 'b', cfg.proxy_b));
            [avg_qoe_mc(trial, a), ~, ~] = qoe(cfg, gamma, cfg.m_k, xi, weights);
        end
    end
