...
for trial = 1:mc
    ...
    for a = 1:numel(algorithms)
        ...
        assign = pick_assignment(cfg, ch, geom, alg, p_dbw, semantic_mode, table_path, weights, trial_seed);
        ...
        sum_rate(trial, a) = out.sum_rate_bps;
        ...
        avg_qoe(trial, a) = out.avg_qoe;

        deadline_vio(trial, a) = out.delay_vio_rate_all;
        semantic_vio(trial, a) = out.semantic_vio_rate_all;

        fprintf([ ...
            '%s: fp=%s, mean(g)=%.4f, std(g)=%.4f, min(g)=%.4f, max(g)=%.4f, ' ...
            'mean(xi)=%.4f, D(mean/p10/p50/p90)=(%.4f/%.4f/%.4f/%.4f), ' ...
            'avgQ=%.6f, dv=%.4f, sv=%.4f\n'], ...
            alg, assign_fp(trial,a), mean(out.gamma_p), std(out.gamma_p), min(out.gamma_p), max(out.gamma_p), ...
            mean(out.xi), D_mean(trial,a), D_p10(trial,a), D_p50(trial,a), D_p90(trial,a), ...
...
function assign = pick_assignment(cfg, ch, geom, alg, p_dbw, semantic_mode, table_path, weights, seed)
    switch alg
        case 'random'
        ...
        case 'qoe'
            % Proposed: UA-QoE-AO (涓ユ牸AO锛氬叧鑱?RSMA-WMMSE+RIS-MM)
            [assign, ~, ~, ~] = ua_qoe_ao(cfg, ch, geom, p_dbw, semantic_mode, table_path, weights);
        case 'exhaustive'
