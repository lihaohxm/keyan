...
def sweep(
    ...
    for algo in algos:
        ...
        curves[f"avg_qoe_urgent_{algo}"] = []
        curves[f"avg_qoe_normal_{algo}"] = []

    n_urgent, n_normal = 4, 8  # 论文中紧迫用户 1–4、普通用户 5–12

    for x_value in x_list:
        algo_sum_rates: Dict[str, List[float]] = {algo: [] for algo in algos}
        algo_avg_qoe: Dict[str, List[float]] = {algo: [] for algo in algos}
        algo_avg_qoe_urgent: Dict[str, List[float]] = {algo: [] for algo in algos}
        algo_avg_qoe_normal: Dict[str, List[float]] = {algo: [] for algo in algos}
        ...
        for trial in range(mc):
            ...
            for algo in algos:
                ...
                per_user = result.per_user_qoe
                if per_user.shape[0] >= n_urgent + n_normal:
                    algo_avg_qoe_urgent[algo].append(float(np.mean(per_user[:n_urgent])))
                    algo_avg_qoe_normal[algo].append(float(np.mean(per_user[n_urgent : n_urgent + n_normal])))
                else:
                    algo_avg_qoe_urgent[algo].append(result.avg_qoe)
                    algo_avg_qoe_normal[algo].append(result.avg_qoe)
        ...
        for algo in algos:
            ...
            curves[f"avg_qoe_urgent_{algo}"].append(summarize(algo_avg_qoe_urgent[algo]).mean)
            curves[f"avg_qoe_normal_{algo}"].append(summarize(algo_avg_qoe_normal[algo]).mean)
